...
export default function FeaturedPost({ featuredPost }: { featuredPost: BlogType }) {
    return (
        <Link to={`blog/${featuredPost.slug}`}>
            <section aria-label="featured-post" className="h-56 sm:h-72 md:h-84 lg:h-92 relative mx-auto rounded-md my-12 shadow-2xl shadow-secondary dark:shadow-accent/50 hover:scale-101 duration-500">
                ...
                <div className="group absolute inset-0 dark:bg-black/40 hover:bg-black/70 text transition-all duration-500 top-0 rounded-md">
                    <div className="absolute bottom-0 p-6 text-primary">
                        <h2 className="text-4xl group-hover:underline font-bold my-1">{featuredPost.title}</h2>
