  ...
  return (
    <div className="w-full grid md:flex gap-2 bg-accent/10 px-3 py-5 ">
      <div className="flex flex-none gap-1">
        {tags && tags.map(tag => (
          <div key={tag} className="bg-accent px-1 py-0.5 rounded-md flex items-center gap-1">{tag} <X strokeWidth={5} className="h-3 w-3 cursor-pointer hover:text-red-500" /></div>
        ))}
      </div>
      {tags.length < 4 && (
        <input
